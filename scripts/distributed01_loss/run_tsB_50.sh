...
do
  ...
  sleep 5
  ssh -i ~/debian.pem debian@$hC sDIR=$sDIR http=$http s=$s 'bash -s ' < $sDIR/run_tsB_30_msC.sh $http $s &
  ...
  sleep 5
  n_processes
  echo "waiting for dockers $nproc"
  while [ "$nproc" -gt 0 ]
  do
    n_processes
    sleep 20
  done 
  echo "to stop/kill dockers $nproc"
