...
func GetCertificatePaths(ho string) (string, string) {
	return path.Join(certPath, ho+".pem"), path.Join(certPath, ho+"-key.pem")
}

// GetTLSConfig returns a tls config for quic.clemente.io
func GetTLSConfig(ho string) *tls.Config {
	cert, err := tls.LoadX509KeyPair(GetCertificatePaths(ho))
...
func GetRootCA() *x509.CertPool {
	caCertPath := path.Join(certPath, "rootCA.pem")
